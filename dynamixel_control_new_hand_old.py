...
class Dynamixel:
    ...
    def enable_torque(self, id: int, enable: bool = True):
        ...
        dxl_addparam_result = self.groupBulkWrite.addParam(id, self.dxls[id].ADDR_TORQUE_ENABLE, 1, [enable_value]) #"""dxl.LEN_GOAL_POSITION"""
    ...
    def update_PID(self, P: int = 36, I: int = 0, D: int = 0):
        ...
        for id in self.dxls.keys():
            dxl = self.dxls[id]
            dxl_addparam_result = self.groupBulkWrite.addParam(id, dxl.ADDR_P_position, dxl.LEN_PID, [DXL_LOBYTE(P), DXL_HIBYTE(P)])
            if dxl_addparam_result != True:
                print("[ID:%03d] groupBulkWrite addparam failed" % id)
                print("wtf")
                quit()
        ...
        for id in self.dxls.keys():
            dxl = self.dxls[id]
            dxl_addparam_result = self.groupBulkWrite.addParam(id, dxl.ADDR_I_position, dxl.LEN_PID, [DXL_LOBYTE(I), DXL_HIBYTE(I)])
            if dxl_addparam_result != True:
                print("[ID:%03d] groupBulkWrite addparam failed" % id)
                print("wtf2")
                quit()
        ...
        for id in self.dxls.keys():
            dxl = self.dxls[id]
            dxl_addparam_result = self.groupBulkWrite.addParam(id, dxl.ADDR_D_position, dxl.LEN_PID, [DXL_LOBYTE(D), DXL_HIBYTE(D)])
            if dxl_addparam_result != True:
                print("[ID:%03d] groupBulkWrite addparam failed" % id)
                quit()
        # Bulkwrite D values
    ...
    def send_goal(self):
        ...
        for id in self.dxls.keys():
            ...
            #param_goal_position = [DXL_LOBYTE(goal), DXL_HIBYTE(goal), DXL_LOBYTE(goal), DXL_HIBYTE(goal)]
            ...
            dxl_addparam_result = self.groupBulkWrite.addParam(id, dxl.ADDR_GOAL_POSITION, 4, param_goal_position) #"""dxl.LEN_GOAL_POSITION"""
            if dxl_addparam_result != True:
                print("[ID:%03d] groupBulkWrite addparam failed" % id)
                quit()
    ...
    def map_pickle(self, i: int):
        ...
        if self.data[i]["joint_5"] == None:
            # Then we know it is a 2v2
            self.dxls[0].goal_position = self.dxls[0].center_pos + self.convert_rad_to_pos(self.data[i]["joint_1"])
            self.dxls[1].goal_position = self.dxls[1].center_pos + self.convert_rad_to_pos(self.data[i]["joint_2"])
            self.dxls[3].goal_position = self.dxls[3].center_pos + self.convert_rad_to_pos(self.data[i]["joint_3"])
            self.dxls[4].goal_position = self.dxls[4].center_pos + self.convert_rad_to_pos(self.data[i]["joint_4"])
    ...
    def set_speed(self, speed):
        ...
        for id in self.dxls.keys():
            # Add Dynamixel max speed value to the Bulkwrite parameter storage
            dxl_addparam_result = self.groupBulkWrite.addParam(id, self.dxls[id].ADDR_velocity_profile, self.dxls[id].LEN_velocity_profile, param_speed)
            if dxl_addparam_result != True:
                print("[ID:%03d] groupBulkWrite addparam failed" % id)
                print("wtf")
                quit()
...
if __name__ == "__main__":
    ...
    print("Speed done")
    input("Press enter")
    Dynamixel_control.replay_pickle_data(file_location = "Open_Loop_Data/2v2_50.50_50.50_1.1_63",file_name="SW_2v2_50.50_50.50_1.1_63.pkl", delay_between_steps = .05)
