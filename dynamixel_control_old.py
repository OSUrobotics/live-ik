...
class Dynamixel:
    ...
    def add_dynamixel(self, type = "XL-320", ID_number = 0, calibration = [0, 511, 1023], shift = 0):
        ...
        self.dxls[ID_number] = dynamixel_old.Dxl(dyn_dict)
    ...
    def go_to_initial_position(self, file_location="actual_trajectories_2v2", file_name="N_2v2_1.1_1.1_1.1_1.1.pkl"):
        ...
        #'r_prox_pin': -0.75,
        #r_distal_pin': 1.4}
        # Test not using from file
        for id in range(4):
            #print(self.data[i])
            self.dxls[id].goal_position = self.dxls[id].center_pos + self.convert_rad_to_pos(start[id])
            #self.dxls[id].joint_angles_pickle[i]
        #self.map_pickle(0)
        self.send_goal()
        #except:
