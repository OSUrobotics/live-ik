...
class ik_manager:
    ...
    def live_run(self, direction = "NW", hand_name = "2v2", ratios="1.1_1.1_1.1_1.1", trial="1"):
        ...
        try:
            while True:
                ...
                m4 = self.dynamixel_control.dxls[4].read_position_m # Get the position of motor 3 - left to
                ...
                contact_point_l, contact_delta_l = contact.contact_point_calculation([object_pose[0], object_pose[1], current_pose[2]], finger_l_contour_m, [m2, m3, m4], "L", dist_length=.0864, sleeve_length=.0500)
                contact_point_r, contact_delta_r = contact.contact_point_calculation([object_pose[0], object_pose[1], current_pose[2]], finger_r_contour_m, [m0, m1], "R", dist_length=.162, sleeve_length=.050)
                ...
                data_dict = {"obj_pos": [actual_pose[0], actual_pose[1], .05], "obj_or": actual_pose[2], "angles": {"joint_1": m0, "joint_2": m1, "joint_3": m2, "joint_4": m3, "joint_5": m4}}
                ...
                if self.move_complete and not self.done:
                    ...
                    #self.block = True
                    self.move_thread(contact_point_l, direction, contact_point_r, ik_left, ik_right, contact_delta_l, contact_delta_r, m0, m1, m2, m3, m4)
            # Start pickle file
    ...
    def move_thread(self, contact_point_l, direction, contact_point_r, ik_left, ik_right, contact_delta_l, contact_delta_r, m0, m1, m2, m3, m4):
        dy = threading.Thread(target=self.dyn_move, args=(contact_point_l, direction, contact_point_r, ik_left, ik_right, contact_delta_l, contact_delta_r, m0, m1, m2, m3, m4,), daemon=True)
        dy.start()

    def dyn_move(self, contact_point_l, direction, contact_point_r, ik_left, ik_right, contact_delta_l, contact_delta_r, m0, m1, m2, m3, m4):
        limit = .6
        ...
        while not self.move_complete:
            ...
            while True:
                ...
                current_4 = self.dynamixel_control.dxls[4].read_position_m # Get the position of motor 3 - left top
                # m0 is old position
                ...
                if goal_diff_0 < targ and goal_diff_1 < targ and goal_diff_2 < targ and goal_diff_3 < targ and goal_diff_4 < targ:
                    # Force a larger step size
                    step += .01
                    break 


                if max(diff_0, diff_1, diff_2, diff_3, diff_4) < targ:
...
if __name__ == "__main__":
    ...
    manager.live_run(direction="SE", hand_name="2v3", ratios="25.75_25.35.40_1.1_53", trial="3")
