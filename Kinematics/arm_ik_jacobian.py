...
def practice_jacobian():
    ...
    # BEGIN SOLUTION
    r = [radius * np.cos(theta), radius * np.sin(theta), 0]
    # END SOLUTION
    ...
    # BEGIN SOLUTION
    jacobian_matrix[0:2, 0] = np.transpose(omega_cross_r[0:2])
    # END SOLUTION
...
def calculate_jacobian_numerically(arm, angles):
    ...
    for i, ang in enumerate(angles):
        # Add the h to the angle
        if i < len(angles) - 1:
            angles[i] += h
        else:
            angles[-1][0] += h
        # Calculate the changed location
        afk.set_angles_of_arm_geometry(arm, angles)
        gripper_loc_plus_h = afk.get_gripper_location(arm)
        # Undo the + h
        if i < len(angles) - 1:
            angles[i] -= h
        else:
        ...
        # Calculate (f(x+h) - f(x)) / h and append the answer to the list
        for c in range(0, 2):
            jacob[c, i] = (gripper_loc_plus_h[c] - gripper_loc[c]) / h
...
def calculate_jacobian(arm, angles):
    ...
    # BEGIN SOLUTION
    angles_links = [a for a in angles[0:-1]]
    angles_links.append(angles[-1][0])
    angles_links.reverse()
    lengths_links.reverse()
    # END SOLUTION
    ...
    for i, (ang, length) in enumerate(zip(angles_links, lengths_links)):
        ...
        omega_cross_r = np.cross(omega_hat, r)
        jacob[0:2, len(angles_links) - i - 1] = np.transpose(omega_cross_r[0:2])
        # END SOLUTION
    return jacob
...
def jacobian_follow_path(arm, angles, target, b_one_step=True):
    ...
    while b_keep_going and count_iterations < 1000:
        ...
        # BEGIN SOLUTION
        afk.set_angles_of_arm_geometry(arm, angles)
        jacobian1 = calculate_jacobian(arm, angles)
        jacobian2 = calculate_jacobian_numerically(arm, angles)
        delta_angles = solve_jacobian(jacobian2, vec_to_target)
        # END SOLUTION
        ...
        while step_size > 0.005 and not b_took_one_step:
            ...
            new_angles.append([angles[-1][0] + step_size * delta_angles[-1], 0, 0])
            # END SOLUTION
