...
def set_transform_base(obj_wedge, base_width=1.0, base_height=0.5):
    ...
    # BEGIN SOLUTION
    obj_wedge["Matrix seq"].append(mt.make_scale_dict(base_height / 2.0, base_width / 2.0))
    obj_wedge["Matrix seq"].append(mt.make_rotation_dict(np.pi/2))
    obj_wedge["Matrix seq"].append(mt.make_translation_dict(0.0, base_width / 4.0))
    # END SOLUTION
...
def set_transform_link(obj_square, arm_length, arm_height):
    ...
    # BEGIN SOLUTION
    obj_square["Matrix seq"].append(mt.make_scale_dict(arm_length / 2.0, arm_height / 2.0))
    obj_square["Matrix seq"].append(mt.make_translation_dict(arm_length * 0.5, 0))
    # END SOLUTION
...
def set_transform_palm(obj_square, palm_width):
    ...
    # TODO: append transformations to obj_square["Matrix seq"] to get it in the right position/size/orientation
    # BEGIN SOLUTION
    obj_square["Matrix seq"].append(mt.make_scale_dict(palm_width / 20.0, palm_width / 2))
    # END SOLUTION
...
def set_transform_finger(obj_wedge, palm_width, finger_size, b_is_top):
    ...
    # BEGIN SOLUTION
    obj_wedge["Matrix seq"].append(mt.make_scale_dict(finger_size[0] / 2.0, finger_size[1] / 2))
    if b_is_top:
        scl = 1.0
    else:
        scl = -1.0
    obj_wedge["Matrix seq"].append(mt.make_translation_dict(finger_size[0] * 0.5, scl * palm_width / 2.0))
    # END SOLUTION
...
def get_gripper_orientation(arm_with_angles):
    ...
    matrices = get_matrices_all_links(arm_with_angles)
    matrix_gripper = matrices[-1] @ get_rotation_link(gripper[0])
    axs_x, _ = mt.get_axes_from_matrix(matrix_gripper)
    return (axs_x[0], axs_x[1])
    # END SOLUTION
    # Format for returning a tuple
    return (1, 0)
