...
def get_sx_sy_from_matrix(mat):
    ...
    # BEGIN SOLUTION
    x_vec = np.zeros(shape=(3,))
    y_vec = np.zeros(shape=(3,))
    x_vec[0] = 1.0
    y_vec[1] = 1.0
    x_vec_scaled = mat @ x_vec
    y_vec_scaled = mat @ y_vec
    return np.linalg.norm(x_vec_scaled), np.linalg.norm(y_vec_scaled)
    # END SOLUTION
...
def check_is_rotation(mat, b_print=False):
    ...
    # BEGIN SOLUTION
    x_axis_rotated, y_axis_rotated = get_axes_from_matrix(mat)

    if not np.isclose(np.linalg.norm(x_axis_rotated), 1.0):
        if b_print:
            print(f"Rotated x axis not unit length {x_axis_rotated}")
        return False

    if not np.isclose(np.linalg.norm(y_axis_rotated), 1.0):
        if b_print:
            print(f"Rotated y axis not unit length {y_axis_rotated}")
        return False

    if not np.isclose(np.dot(x_axis_rotated, y_axis_rotated), 0.0):
        print(f"Axes not orthogonal {x_axis_rotated}, {y_axis_rotated}")
        return False

    mat_ident = mat @ np.transpose(mat)
    if not np.all(np.isclose(mat_ident, np.identity(3), 0.0001)):
        return False
...
def check_is_mirrored(mat):
    ...
    z_axis = np.cross(x_axis_rotated, y_axis_rotated)
    if z_axis > 0.0:
        return False
    return True
    # END SOLUTION
...
def check_preserves_angles(mat):
    ...
    # BEGIN SOLUTION
    x_axis_rotated, y_axis_rotated = get_axes_from_matrix(mat)
    dot_product = np.dot(x_axis_rotated, y_axis_rotated) / (np.linalg.norm(x_axis_rotated) * np.linalg.norm(y_axis_rotated))
...
def example_weird_geometry():
    ...
    # BEGIN SOLUTION
    seq_mirrored = [{"type":"scale", "sx": -2.0, "sy": 1.0},
...
def example_uncentered_geometry():
    ...
    pts_circle_x_up_lower_left_origin = make_translation_matrix(1, 1) @ make_rotation_matrix(-np.pi/2.0) @ pts_circle
    pts_zigzag_x_up_lower_left_origin = make_translation_matrix(1, 1) @ make_rotation_matrix(-np.pi/2.0) @ pts_zigzag
    # END SOLUTION
