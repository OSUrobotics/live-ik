...
def vector_to_goal(arm_with_angles, target):
    ...
    # BEGIN SOLUTION
    end_pt = afk.get_gripper_location(arm_with_angles)
...
def calculate_gradient(arm, angles, target):
    ...
    dist_fx = distance_to_goal(arm, target)
    ...
    for i, ang in enumerate(angles):
        ...
        afk.set_angles_of_arm_geometry(arm, angles)
        dist_fx_plus_h = distance_to_goal(arm, target)
        # Undo the + h
        if i < len(angles) - 1:
            angles[i] -= h
        else:
        ...
        derivs.append((dist_fx_plus_h - dist_fx) / h)
    ...
    while b_keep_going and count_iterations < 1000:
        ...
        while step_size > 0.005 and not b_took_one_step:
            ...
            for i, a in enumerate(angles[:-1]):
                new_angles.append(a - step_size * gradient[i])
            new_angles.append([angles[-1][0] - step_size * gradient[-1], 0, 0])
            # END SOLUTION
...
if __name__ == '__main__':
    ...
    test_reach_target(gradient_descent)
    # END SOLUTION
